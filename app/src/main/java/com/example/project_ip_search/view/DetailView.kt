...
import com.example.project_ip_search.components.MainImage
...
@Composable
fun DetailView(viewModel: IdViewModel, navController: NavController, id: String) {
    LaunchedEffect(Unit) {
    ...
    Scaffold(
        topBar = {
            MainTopBar(title = viewModel.state.ip, showBackButton = true, onClickBackButton = {
...
@Composable
fun ContentDetailView(pad: PaddingValues, viewModel: IdViewModel) {
    val state = viewModel.state
    Log.d("games2", "detailView ${state.toString()}")
    Column(
        modifier = Modifier
            .padding(pad)
            .background(Color(CUSTOM_BLACK))
    ) {
        /*MainImage(image = state.ip)
